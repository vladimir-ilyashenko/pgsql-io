...
./$api key insert $USER-key ubuntu ~/keys/$USER-key.pem
...
./$api cloud create openstack nnj2 --region=nnj2 --keys=$HOME/.openstack/nnj2.env --default-ssh-key=$USER-key
./$api cloud create openstack nnj3 --region=nnj3 --keys=$HOME/.openstack/nnj3.env --default-ssh-key=$USER-key
